package com.media.mfcloud.video;
...
import com.media.mfcloud.R.id;
import com.media.mfcloud.R.layout;

public class VideoStream extends Activity implements OnSeekBarChangeListener, Callback, OnPreparedListener, OnCompletionListener, OnBufferingUpdateListener,
	...
	public void playVideo() {
		if (extras.getString("video_url").equals("VIDEO_URI")) {
			...
			new Thread(new Runnable() {
				public void run() {
					try {
						player.setDisplay(holder);
						player.setDataSource(VideoStream.this,Uri.parse(extras.getString("video_url")));
						player.prepare();
					...
					showToast("Error while playing video1");
					Log.i(TAG, "========== IllegalArgumentException ===========");
					e.printStackTrace();
				} catch (IllegalStateException e) {
					showToast("Error while playing video1");
					...
					showToast("Error while playing video. Please, check your network connection111.");
					Log.i(TAG, "========== IOException ===========");
					e.printStackTrace();
				}*/
	...
	private void showToast(final String string) {
		runOnUiThread(new Runnable() {
			public void run() {
				Toast.makeText(VideoStream.this, string, Toast.LENGTH_LONG).show();
				finish();
			}
		});
	}
